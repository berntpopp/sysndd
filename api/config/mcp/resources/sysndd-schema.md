...
Payload controls: `response_mode` is `compact`, `standard`, or `full`; `abstract_mode` is `none`, `metadata`, or `excerpt`; `synopsis_mode` is `none`, `excerpt`, or `full`. Use `abstract_mode:"metadata"` for citation lists and `abstract_mode:"excerpt"` only when summarizing abstract content. `include_comparisons` defaults to false on `get_gene_context`.

Use `get_publication_context` for one PMID and `get_publications_context` for 1-20 PMIDs. Publication outputs include `recommended_citation`, `pubmed_publication_date`, `pubmed_publication_date_confidence`, `pubmed_publication_date_matches_curation_date`, and, depending on `abstract_mode`, `abstract_available`, `abstract_excerpt`, and `abstract_truncated`. Linked entity review dates are exposed as `sysndd_curation_date`.
