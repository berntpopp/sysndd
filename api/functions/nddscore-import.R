# api/functions/nddscore-import.R
#
# NDDScore import transaction - advisory lock, DB upserts/inserts/counts,
# activation, failure classification, and the `nddscore_run_import()`
# orchestrator. No secrets are logged. See db/migrations/023 for the
# target schema.
#
# Release acquisition (config resolution, Zenodo metadata/download/
# checksum/extract, JSON/TSV parsing, schema validation) lives in the
# sibling functions/nddscore-release-source.R (#346 split; must load
# first — `nddscore_run_import()`'s default `deps` resolve those
# functions by name). Guard-source it below so any entrypoint (unit
# tests, API/worker bootstrap) that sources this file directly still
# gets it. Resolve relative to THIS file's own directory (active
# source() frame), not cwd, since a plain cwd-relative path silently
# fails under testthat::test_dir() (see helper-functions.R, same idiom).
if (!exists("nddscore_fetch_zenodo_metadata", mode = "function")) {
  .nddscore_import_dir <- local({
    ...
    else tryCatch(file.path(get_api_dir(), "functions"), error = function(e) "functions")
  })
  .nddscore_release_source_path <- file.path(.nddscore_import_dir, "nddscore-release-source.R")
  if (file.exists(.nddscore_release_source_path)) {
    source(.nddscore_release_source_path, local = FALSE)
  }
  rm(.nddscore_import_dir, .nddscore_release_source_path)
}
