# api/functions/nddscore-release-source.R
#
# NDDScore release acquisition - pure, unit-testable functions.
# Fetch / download / verify / extract / parse a Zenodo release, plus
# schema validation of the extracted content. No DB writes here; no
# secrets are logged. See db/migrations/023 for the target schema.
#
# Extracted from nddscore-import.R (#346) to keep both files under the
# repository's 600-line soft ceiling. nddscore-import.R keeps the
# advisory lock, DB upserts/inserts/counts/activation/failure helpers,
# and the `nddscore_run_import()` orchestrator, which calls back into
# the functions defined here (fetch/download/verify/extract/parse/
# load/validate) via its default `deps` list.
