...
#
# State persistence (schema_version table creation, manifest file listing,
# applied-row reads, rename reconciliation, and record insertion) lives in
# the sibling functions/migration-state-repository.R (#346 Wave 4 split, to
# keep both files under the repository's 600-line ceiling). This file keeps
# the advisory lock, SQL statement splitter/executor, the pending-migration
# diff, and the run_migrations() entry point.
#
# Usage:
...
# Ensure the extracted state-persistence functions are available:
# ensure_schema_version_table(), list_migration_files(),
# get_applied_migrations(), MIGRATION_RENAMES,
# reconcile_schema_version_renames(), record_migration(). load_modules.R's
# explicit source list is NOT changed by this split (deliberately, to keep
# this refactor scoped to functions/*) — migration-state-repository.R is
# instead guard-sourced right here, so every entrypoint that sources
# migration-runner.R (load_modules.R's bootstrap, or a direct-source test/
# db-helper fixture) transitively gets the state functions in the effective
# order manifest -> state repository -> runner. Resolve the sibling file
# relative to THIS file's own directory (derived from the active source()
# frame), mirroring functions/helper-functions.R, rather than a cwd-relative
# guess that only resolves under some test invocations (see the sibling
# comparisons-functions.R guard-source trap).
if (!exists("ensure_schema_version_table", mode = "function")) {
  .migration_runner_dir <- local({
    ofiles <- Filter(Negate(is.null), lapply(sys.frames(), function(f) f$ofile))
    if (length(ofiles) > 0) {
      dirname(normalizePath(ofiles[[length(ofiles)]]))
    } else if (file.exists("functions/migration-state-repository.R")) {
      "functions"
    } else if (file.exists("/app/functions/migration-state-repository.R")) {
      "/app/functions"
    } else {
      "functions"
    }
  })
  .migration_state_repo_path <- file.path(.migration_runner_dir, "migration-state-repository.R")
  if (file.exists(.migration_state_repo_path)) {
    source(.migration_state_repo_path, local = FALSE)
  }
  rm(.migration_runner_dir, .migration_state_repo_path)
}
