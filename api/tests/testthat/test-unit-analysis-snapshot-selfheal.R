...
test_that("the read service resolves the self-heal by NAME (production wiring, no injection)", {
  # The three tests above inject `submit_fn`/`on_stale_refresh`; production does
  # neither. `service_analysis_snapshot_read()` resolves the trigger lazily with
  # `get0(..., mode = "function")` and the trigger resolves `submit_fn` from its
  # call-time default -- both walk the global env the API sources modules into.
  # Guard that lookup explicitly: `config` (attached in the real API/worker env)
  # masks `base::get` with a signature that has NO `mode` argument, so a future
  # refactor to `get(...)` would break only in production. Regression cover for
  # the same masking class that broke every #514 snapshot refresh.
  skip_if_not_installed("config")
  suppressMessages(library(config))
  on.exit(try(detach("package:config", unload = FALSE), silent = TRUE), add = TRUE)

  # The env the modules were sourced into -- production's globalenv analogue and
  # the env both name lookups walk to.
  module_env <- environment(service_analysis_snapshot_selfheal_on_serve)
  expect_identical(module_env, environment(service_analysis_snapshot_read))

  fired <- 0L
  prior <- base::get("service_analysis_snapshot_submit_refresh", envir = module_env)
  assign("service_analysis_snapshot_submit_refresh", function(force = TRUE, stagger = TRUE, ...) {
    fired <<- fired + 1L
    selfheal_fake_summary()
  }, envir = module_env)
  on.exit(assign("service_analysis_snapshot_submit_refresh", prior, envir = module_env), add = TRUE)
  # Clear the process-wide throttle so this call is not swallowed by an earlier one.
  .analysis_snapshot_selfheal_state$last_submit_epoch <- NULL
  on.exit(.analysis_snapshot_selfheal_state$last_submit_epoch <- NULL, add = TRUE)

  result <- suppressMessages(service_analysis_snapshot_read(
