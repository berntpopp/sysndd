...
test_that("svc_user_password_reset_request parses the body, collects users, and forwards the status", {
  ...
  res <- make_mock_res()
  result <- env$svc_user_password_reset_request(list(postBody = '{"email":"a@x.com"}'), res)
...
test_that("svc_user_password_reset_request defaults to an empty email on malformed JSON (never 500s)", {
  captured <- NULL
  env <- make_service_sandbox(
  ...
  res <- make_mock_res()
  result <- env$svc_user_password_reset_request(list(postBody = "not json"), res)
