...
test_that("get_gene_context reports total entity count when entity rows are omitted", {
  source("../../functions/mcp-repository.R")
  source("../../services/mcp-service.R")
  ...
  assign("mcp_repo_get_gene_entities", function(...) {
    stop("entity rows should not be fetched when include_entities is false")
  }, envir = .GlobalEnv)
  assign("mcp_repo_count_gene_entities", function(...) 7L, envir = .GlobalEnv)
  assign("mcp_repo_get_gene_comparisons", function(...) tibble::tibble(), envir = .GlobalEnv)
  withr::defer({
  ...
  result <- mcp_get_gene_context("GRIN2A", include_entities = FALSE)

  expect_equal(result$entity_summary$entity_count, 7L)
  expect_equal(result$meta$entity_total, 7L)
  expect_equal(result$meta$entity_returned, 0L)
  expect_false(result$meta$entity_rows_included)
})
