...
test_that("manifest expects migration 033 as latest", {
  expect_equal(EXPECTED_LATEST_MIGRATION, "033_add_metadata_lookup_admin_columns.sql")
...
test_that("migration manifest validates against db/migrations", {
  ...
  expect_true(res$ok)
  expect_identical(res$latest, "033_add_metadata_lookup_admin_columns.sql")
})
