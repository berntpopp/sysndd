    ...
    <BCardBody>
      ...
      <div v-if="systems.length" class="mb-2">
        ...
        <div class="system-chips">
          <BBadge v-for="row in systems" :key="row.system" variant="light" class="system-chip">
      ...
      <p class="provenance-note mb-0">
        ...
        Computed from curated HPO annotations (rule {{ scalarOf(block.rule_version) }}). Counts
        distinct organ systems outside the nervous system; intellectual disability, nervous-system
        findings, head size and clinical-course terms are excluded. &ldquo;None recorded&rdquo;
        means no such system is documented, which is not the same as confirmed absence.
      </p>
    </BCardBody>
  </BCard>
</template>
