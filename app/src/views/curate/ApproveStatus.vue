...
async function handleStatusOk(_evt: unknown): Promise<void> { try { await ax().put(`${apiBase()}/api/status/approve/${status_info.value.status_id}?status_ok=true`, {}, { headers: authHdr() }); announce('Status approved successfully'); loadStatusTableData(); } catch (e) { makeToast(e, 'Error', 'danger'); announce('Error approving status', 'assertive'); } }
async function handleDismissOk(_evt: unknown): Promise<void> { try { await ax().put(`${apiBase()}/api/status/approve/${dismissModal.statusId}?status_ok=false`, {}, { headers: authHdr() }); announce('Status dismissed successfully'); loadStatusTableData(); } catch (e) { makeToast(e, 'Error', 'danger'); announce('Error dismissing status', 'assertive'); } }
async function submitStatusChange(): Promise<void> { isBusy.value = true; try { status_info.value.status_user_name = null; status_info.value.status_user_role = null; status_info.value.entity_id = null; await ax().put(`${apiBase()}/api/status/update`, { status_json: status_info.value }, { headers: authHdr() }); const m = 'The new status for this entity has been submitted successfully.'; makeToast(m, 'Success', 'success'); announce(m); status_info.value = new Status() as unknown as StatusInfoShape; loadStatusTableData(); } catch (e) { makeToast(e, 'Error', 'danger'); announce('Error submitting status', 'assertive'); } finally { isBusy.value = false; } }
async function handleAllStatusOk(): Promise<void> { if (!approve_all_selected.value) return; try { await ax().put(`${apiBase()}/api/status/approve/all?status_ok=true`, {}, { headers: authHdr() }); loadStatusTableData(); } catch (e) { makeToast(e, 'Error', 'danger'); } }
