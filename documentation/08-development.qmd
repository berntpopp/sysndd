...
Cache-backed analysis sections such as phenotype clusters and STRING-derived gene networks require prewarmed API cache entries. The MCP sidecar mounts the shared API cache read-only and reports `temporarily_unavailable` when the exact cache key is missing; use the API/admin analysis path to warm those artifacts during development.
