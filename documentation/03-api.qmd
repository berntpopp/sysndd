...
The Phase 0 transport spike proved `mcptools` HTTP initialize -> `tools/list` -> `tools/call`, `GET 405`, no required session header, and JSON-serialized text tool output. MCP v1 therefore returns stable JSON text with `schema_version` rather than relying on native `structuredContent`.
