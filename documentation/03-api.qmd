...
The static `sysndd://schema/overview` and `sysndd://schema/tool-guide` resources document schema and tool usage. Record-level `sysndd://gene`, `sysndd://entity`, and `sysndd://publication` URIs in tool payloads are stable identifiers; v1 retrieval remains tool-based rather than parameterized resource templates.
...
The Phase 0 transport spike proved `mcptools` HTTP initialize -> `tools/list` -> `tools/call`, `GET 405`, no required session header, and JSON-serialized text tool output. MCP v1 therefore returns stable JSON text with `schema_version` rather than relying on native `structuredContent`. The sidecar patches `mcptools` initialize metadata so clients receive SysNDD-specific workflow instructions instead of the package's generic R-session instructions.
