...
MCP v1 is read-only, public and credential-free by design when the opt-in `mcp` Compose profile is enabled. Production Traefik routes exact JSON-RPC `POST /mcp` requests to the sidecar and strips the prefix; a narrowly matched `GET /mcp` with `Accept: text/event-stream` and exact `DELETE /mcp` reach the transport and currently return `405` because standalone SSE listening and session termination are not offered. Normal browser `GET /mcp` navigation continues to render the informational Vue page. Admission controls are one global bucket shared across callers behind the institutional TLS proxy, so one client can temporarily exhaust capacity for everyone; clients should treat `429` as shared-capacity backpressure and retry with backoff. Public reachability does not widen the approved-public data contract described below, and the sidecar has no outbound internet egress.

The pinned `mcptools` runtime implements the initialize/session-era Streamable HTTP shape. SysNDD's operational probes negotiate MCP `2025-11-25` and fail if initialize returns a different version; production also accepts `2025-06-18` clients. The latest upstream revision as of 2026-08-27 is `2026-07-28`, whose stateless core removes the initialize/session era and adds per-request protocol metadata headers. Issue #629 changes public reachability and edge controls only; it does not claim support for that breaking revision. Upgrade `mcptools`, the health/smoke clients, and protocol compatibility tests together before advertising `2026-07-28`. Current clients should use MCP's specified backward-compatibility fallback.
...
`MCP_SCHEMA_VERSION` `1.2` adds a bounded analysis scope for gene-centered research context. Start with `get_sysndd_analysis_catalog`, then use `get_gene_research_context` with MCP JSON arguments such as `{"gene":"HGNC:61","dry_run":true,"response_mode":"compact"}` to preflight available sections before focused follow-up calls. The focused tools expose curation comparisons, NDDScore predictions, phenotype analyses, and local gene network context. They do not change curated SysNDD evidence.

Every MCP analysis payload carries a data-class label: `curated_sysndd_evidence`, `curated_derived_analysis`, `ml_prediction`, `llm_generated_summary`, `external_reference_identifier`, or `operational_metadata`. NDDScore is always `ml_prediction`: model-derived, separate from curated SysNDD evidence, and not an evidence tier. Cached LLM summaries are `llm_generated_summary`: current validated admin-generated cache entries only. MCP analysis tools do not trigger Gemini/LLM generation, expose prompts or queries, or call live external providers. Stored external IDs may appear only as `external_reference_identifier`.

Payload controls are part of the MCP contract: `response_mode` is `minimal`, `compact`, `standard`, `full`, or analysis-only `diagnostics`; `max_response_chars` defaults to `auto` for large analysis tools; `abstract_mode` is `none`, `metadata`, or `excerpt`; and `synopsis_mode` is `none`, `excerpt`, or `full`. `minimal` defaults to no synopsis and no abstracts. Large analysis responses default to compact mode, include `budget` metadata, and may report `dropped_summary`, `recovery`, and per-section availability. Use `dry_run` or `response_mode = "diagnostics"` to inspect likely sections, truncation, and recovery hints before requesting broader context. Other modes default to citation metadata rather than prose; request `excerpt` or `full` only when text is needed. Tool results report `meta.elapsed_ms`. Entity phenotypes are grouped as modifier-keyed HPO ID arrays, and batch/expanded payloads keep `schema_version` only at the outer envelope. `abstract_mode = "metadata"` reports `abstract_available` without `abstract_excerpt` or `abstract_truncated`. `get_gene_context` defaults `include_comparisons` to false for the cheap path and reports first-page entity metadata in `meta.entity_total`, `meta.entity_returned`, `meta.entity_has_more`, and `meta.next_entity_offset`. `expand = "entities"` can return one-call gene plus entity detail, with detailed expansion capped at 20 entity IDs and `meta.entity_detail_truncated_by_batch_cap` reporting when that cap was applied. `get_entities_context` defaults `dedupe_publications` to true, moving shared publication objects to a top-level `publications` list and leaving per-entity `publication_refs`.
...
The MCP service exposes only approved-public data: active records represented by `ndd_entity_view` and review-derived evidence from primary approved reviews (`is_primary = 1` and `review_approved = 1`). It must not write to the database, call write routes, execute raw SQL/R, expose admin/user/log/job data, expose draft reviews or re-review workflows, call live external providers, or trigger Gemini/LLM generation.
