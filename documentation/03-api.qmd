...
MCP v1 is read-only, public and credential-free by design when the opt-in `mcp` Compose profile is enabled. Production Traefik routes exact JSON-RPC `POST /mcp` requests to the sidecar and strips the prefix; a narrowly matched `GET /mcp` with `Accept: text/event-stream` reaches the transport and currently returns `405` because standalone SSE listening is not offered. Normal browser `GET /mcp` navigation continues to render the informational Vue page. Admission controls are shared across callers behind the institutional TLS proxy, so clients should treat `429` as shared-capacity backpressure and retry with backoff. Public reachability does not widen the approved-public data contract described below, and the sidecar has no outbound internet egress.
