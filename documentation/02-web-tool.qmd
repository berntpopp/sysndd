...
Multiple correspondence analysis (MCA) is performed utilizing the "MCA" function from the [FactoMineR](http://factominer.free.fr/) R package, with the number of dimensions retained set to 15, the qualitative supplementary variables set to inheritance terms, and the quantitative supplementary variables set to three registry-derived counts per entity: the number of intellectual-disability terms, the number of distinct organ systems involved outside the nervous system, and the number of disease entities per gene. Then, hierarchical clustering is performed using the "HCPC" function from the FactoMineR package.
