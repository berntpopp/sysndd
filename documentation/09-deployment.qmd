...
- `ASYNC_JOB_RETENTION_DAYS=90` — delete **terminal, non-retryable** `async_jobs` rows (`status IN ('completed','failed','cancelled')` and `active_request_hash IS NULL`) older than this many days (by both `submitted_at` and `updated_at`), in batches; `async_job_events` cascades. `ASYNC_JOB_RETENTION_DRY_RUN=1` counts only.
