...
- The API uses `mirai` workers for background tasks.
...
- Run the worker service alongside the API service; jobs are executed by the worker entrypoint, not the web process.
