...
request-payload hash of `hgnc_update`/`comparisons_update`/`omim_update`. These
destructive full-table-replace jobs now dedupe by **job type** (single-flight),
so steady state is safe, but during the single deploy that lands S2b an
in-flight pre-deploy job (old hash) will not dedupe against a newly-submitted
post-deploy job. Prefer draining any in-flight maintenance job before deploying
(these are admin/cron-triggered singletons; each is a full replace, so even a
rare overlap converges to a consistent state). The scrub only touches terminal,
