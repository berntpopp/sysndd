...
Database backup/restore jobs never carry the DB password in the durable job
payload, in process argv, or in a shell command string:

- `svc_backup_create`/`svc_backup_restore` submit **credential-free** params; the
  durable handlers resolve the credential at run time from the worker's `dw`
  config via `async_job_worker_db_config()`.
- `execute_mysqldump`/`execute_restore` pass the password to the MySQL CLIs
  through a per-invocation **mode-0600 `--defaults-extra-file`** (created
  fail-closed), never `-p<password>` argv or an interpolated `system()` shell
  string.
- Historical terminal backup payloads (and their password-derived
  `request_hash`) are redacted idempotently by
  `async_job_scrub_payload_credentials()`, which runs best-effort at API startup,
  after every restore, and via the operator script.

**Mandatory operator steps after deploying this change** (prior backups, logs,
and payloads may already contain the password, so the code fix alone is not
sufficient):
...
Rotation is the primary mitigation for any credential that leaked before this
change — the scrub cannot rewrite old backup files. The scrub also runs after
every restore (an old dump can re-import credential-bearing rows). Its outcome
is logged at WARN in the worker log — the **reliable** signal, since a *full*
restore replaces `async_jobs` and can drop the restore job's own row (so the
`result_json.post_restore_scrub` field is best-effort; this is the restore-
fencing limitation tracked separately as S3). The next API-startup scrub is an
idempotent backstop. Set `ASYNC_JOB_PAYLOAD_SCRUB_ON_STARTUP=false` only to
disable the startup scrub (default on). As of #535 S2b, **all** durable job
families (publication, hgnc/comparisons/omim/force_apply, provider,
pubtator/pubtatornidd, llm) resolve DB credentials at run time via
`async_job_db_connect()` — no payload carries `db_config`. The scrub is
correspondingly job-type agnostic and redacts both `$.db_config.password` and
`$.db_config.db_password` for historical terminal rows.

**Deploy note (S2b payload-schema change):** dropping `db_config` changes the
request-payload hash of `hgnc_update`/`comparisons_update`/`omim_update`/
`force_apply_ontology`. These destructive full-table-replace jobs now dedupe by
**job type** (best-effort submit-time single-flight), which returns a clean 409
for a same-type resubmit even across the deploy. Concurrency safety for
destructive execution does **not** rely on this dedup: the durable **maintenance
lane runs on a single worker** that claims and runs jobs strictly sequentially
(one `worker-maintenance` container), so two destructive jobs never execute
concurrently — even if a rare non-atomic double-submit enqueues two. The scrub
only touches terminal, non-retryable rows, so it never races an active job.

If you ever scale the maintenance lane beyond one worker, add a hard, atomic
**cross-type conflict-group mutex** first (advisory lock over check+insert, or a
generated conflict-key unique index grouping `omim_update`/`force_apply_ontology`
on `disease_ontology_set` and `pubtator_enrichment_refresh`/`pubtatornidd_nightly`
on the enrichment snapshot) — the submit-time single-flight alone is not
sufficient for concurrent workers. Draining in-flight maintenance jobs before
deploying is prudent regardless.

### `MIRAI_WORKERS`

Controls legacy mirai daemon worker count. Background asynchronous jobs are drained by durable MySQL-backed workers (`worker` and `worker-maintenance` containers) rather than the mirai daemon pool.

- Default: `0` (disabled, saving ~630 MB RAM per daemon on API containers)
- If enabled:
  - small server: `1`
  - medium server: `2`
  - large server: `4`

Rule of thumb when non-zero:
...
Controls the database connection pool in the API container.

Recommended baseline:

- `MIRAI_WORKERS=0` -> `DB_POOL_SIZE=3-5`
...
**Phenotype clustering owns k and runs a multi-start consolidation** (#679): `gen_mca_clust_obj()` no longer calls `FactoMineR::HCPC`. `api/functions/analysis-phenotype-consolidation.R` builds the Ward tree on the MCA coordinates, selects k in 3..25 by the minimum of W(k)/W(k-1), and consolidates with Hartigan-Wong k-means from the Ward-cut centroids **and** from seeded random starts, releasing the lowest within-cluster inertia (ties: Ward-cut start, then seed order). A single Ward-cut start had converged to the higher-inertia of two optima on production data, and a one-entity change could flip the released partition between them. `ANALYSIS_PHENOTYPE_CONSOLIDATION_STARTS` (default `100`; `0` reproduces the legacy single-start procedure) sets the number of random starts; it changes membership and is therefore part of the phenotype cache fingerprint, so set it identically on `api`, `worker` and `worker-maintenance`. `partition_validation` gains `procedure_version`, `k_rule`, `k_ward_ratio_curve`, `k_selection` (runner-up k and ratio margin: multi-start stabilises the partition *at* a given k, so a near-zero margin is the remaining way a small input change can re-split the entities), `consolidation_landscape` (starts, basins, chosen vs. runner-up inertia, whether the Ward-cut start reached the chosen basin), `continuity` (adjusted Rand index against the public snapshot being superseded) and `factominer_version`; all are excluded from `payload_hash`. They are part of `validation_json`, so `validation_hash` changes on every refresh (`continuity.previous_snapshot_id` alone guarantees it) — do not read a changed `validation_hash` as a changed partition; `payload_hash` / `cluster_hash` are the membership signals. The MCA is always computed from the full spectrum (`phenotype_mca_fit()`), never by passing `ncp = 8` to FactoMineR, and the FactoMineR version is part of the phenotype cache fingerprint. **This is a membership change**: it ships with a `CLUSTER_LOGIC_VERSION` bump, so follow the membership-change runbook — restart `worker` + `worker-maintenance`, force-refresh `phenotype_clusters`, then force-refresh `phenotype_functional_correlations` (its dependency gate pins the phenotype `snapshot_id`), then force-regenerate the phenotype LLM summaries (their `cluster_hash` changes). Cluster numbers are assigned by ascending first-MCA-coordinate centroid and may differ from the previous snapshot. Existing snapshots and published releases are not rewritten. FactoMineR now supplies only `MCA` and `catdes`; `test-unit-phenotype-package-drift-guard.R` must stay green, unchanged, across any FactoMineR or R update.
...
1. Ensure the exp+db artifact exists (`GET /api/health` → `analysis.expdb_edges_file_present: true`), building it as above if needed.
...
Dev is single-lane: `make dev` profile-gates `api-enrichment` out (`prod-enrichment-lane`) and pins `api` to one replica, so `/api/external` is served by the single dev API container.
...
- `MCP_ALLOWED_ORIGINS` is a comma-separated exact allowlist for non-empty browser `Origin` headers and defaults to `https://sysndd.dbmr.unibe.ch`. Requests without `Origin` remain valid for server-side MCP clients; empty, malformed, or unlisted values fail closed.
...
Ordinary `docker compose up` excludes MCP. Provision or rotate the SELECT-only
reader only after migration 044 exists:
...
Enabling MCP before `./secrets/mcp-db-password` exists fails closed: Docker
refuses the read-only bind instead of creating a directory at that path.

The provisioner locks/quarantines every reader variant first, uses [MySQL 8.4 server-generated random credentials](https://dev.mysql.com/doc/refman/8.4/en/password-management.html#random-password-generation), attests the exact 23 view definitions/grants/roles, atomically installs the owner-only secret, and unlocks last. It never accepts a desired reader password or places one in SQL, argv, logs, URLs, or payloads. A failed run leaves the reader locked and privilege-free; correct the cause and rerun serially.
...
When the opt-in `mcp` profile is enabled, the production transport is public and credential-free by design. The checked-in Traefik labels route exact JSON-RPC `POST /mcp` requests and `GET /mcp` requests with `Accept: text/event-stream` to the sidecar, while normal browser `GET /mcp` navigation still reaches the public informational Vue page. Both routers bind explicitly to the `mcp` service and strip `/mcp` before forwarding to the `mcptools` HTTP root. The current transport returns `405` for standalone SSE GET; clients should use JSON-RPC POST.

The pinned `mcptools` runtime implements the initialize/session-era Streamable HTTP shape. SysNDD's operational probes negotiate MCP `2025-11-25` and fail if initialize returns a different version; production also accepts `2025-06-18` clients. The latest upstream revision as of 2026-08-27 is `2026-07-28`; it is a breaking stateless revision that removes the initialize/session era and requires per-request protocol metadata such as `MCP-Protocol-Version`, `Mcp-Method`, and (where applicable) `Mcp-Name`. Do not infer `2026-07-28` support from the public edge labels. Treat adoption as a separate compatibility migration: upgrade `mcptools`, update health/smoke clients, keep exact GET/DELETE routing so unsupported methods continue to return `405`, and exercise old/new client fallback before changing the advertised protocol version.

The public edge is deliberately bounded. A shared Host bucket permits 120 requests per minute with a 20-request burst. Because production TLS terminates at an institutional proxy, this is intentionally a single global bucket rather than a per-person quota: one aggressive or malfunctioning client can temporarily exhaust shared capacity for everyone. A rate-limit rejection returns HTTP 429; in-flight overflow also returns HTTP 429. Clients should retry with backoff and cache stable results. POST requests are limited to 256 KiB and 4 concurrent POST requests for the public Host bucket; excess body size returns HTTP 413. Traefik applies a 60-second whole-request read timeout. These controls provide admission and resource bounds, not authorization. If the institutional proxy CIDRs and forwarding behavior become stable operator inputs, configure Traefik's trusted proxy boundary first and then replace the Host source criterion with a tested client-IP strategy; never trust caller-supplied forwarded headers directly.

Traefik and the sidecar share only the internal `mcp_edge` network. MCP also joins `backend` for its attested SELECT-only database principal, but it is not attached to the egress-capable `proxy` network and therefore has no outbound internet egress. This preserves the approved-public, read-only data boundary even though the transport is unauthenticated.

The MCP implementation validates browser `Origin` according to the MCP transport security guidance: the canonical SysNDD origin and requests without an `Origin` header are accepted, while an invalid non-empty `Origin` is rejected with `403`. Origin validation limits browser DNS-rebinding/cross-origin abuse; it does not replace authentication and must not be described as authenticating a caller.

Access logs are enabled only for the three MCP protocol routers; the public app and Traefik's internal API/ping entrypoint are opted out. Entries retain method, path, duration, router, and response status needed to diagnose 403/405/413/429/5xx responses. Bodies, headers, query parameters, `ClientAddr`, and `ClientHost` are dropped, so request content and caller IP addresses are not retained. Log-file retention is bounded by the Traefik container's `json-file` rotation (`10m` × 3 files). Keep these privacy defaults when changing observability. `make test-mcp-edge` is a required pre-deploy gate for routing or middleware changes; the disposable smoke stack exercises browser routing, protocol POST, Accept matching, GET/DELETE `405`, body `413`, rate-limit `429`, router log scope, and sensitive-field omission using the resolved production image, command, and labels.
...
- Keep the public credential-free MCP edge bounded and observable: preserve the shared rate/body/concurrency limits, `mcp_edge` network isolation, invalid-Origin rejection, privacy-minimized access logs, and approved-public SELECT-only database principal. MCP tools and prompts are read-only and must not call Gemini/LLM generation, live external providers, raw SQL/R execution, write routes, admin/user/log/job routes, draft reviews, or re-review data. Analysis tools may read only validated stored summary projections and remain bounded by compact defaults plus `max_response_chars`.
...
## Session security — revocable token refresh (#535 P0-2)

Migration `043_add_user_session_epoch.sql` adds `user.session_epoch` (default `0`) and is applied by the
startup migration runner. Token refresh (`GET /api/auth/refresh`) now loads current account state, requires
`approved == 1`, and requires the token's `sepoch` claim to equal `user.session_epoch`; any privilege/state
change (role change, deactivation, password change) increments the epoch and immediately **revokes the
user's refresh** (they must sign in again).

Operator notes:

- **Pre-deploy tokens** (issued before this deploy) carry no `sepoch` and are **rejected on refresh**, so
  every user signs in once after cutover. Existing *access* tokens keep working until they expire
  (`config$token_expiry`, ~1h); there is no forced immediate logout.
- To additionally invalidate **all** in-flight access tokens at cutover (e.g. after a suspected token leak),
  run `UPDATE user SET session_epoch = session_epoch + 1;` once after the migration — this forces re-login on
  the next refresh for everyone.
- No worker change is needed for this feature; `auth-service.R` runs in the API, so a normal API restart
  (which also applies the migration) is sufficient.
- Deferred follow-ups (**S1b**): immediate access-token revocation (an epoch check in `require_auth`), distinct
  rotating refresh tokens for theft resistance, and repairing the pre-existing `user-service.R` bulk-approve
  `account_status` schema mismatch.
