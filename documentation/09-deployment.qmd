...
Access logs are enabled only for the two MCP protocol routers. They retain request metadata needed to diagnose 413/429/5xx responses, but bodies, headers, and query parameters are not logged. Keep these privacy defaults when changing Traefik observability. Run `make test-mcp-edge` after routing or middleware changes; the disposable smoke stack exercises browser routing, protocol POST, GET `405`, body `413`, and rate-limit `429` using the real Compose labels.
