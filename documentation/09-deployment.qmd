...
The `api`, `worker`, and `worker-maintenance` services bind-mount `./api/data`; the `mcp` service mounts it **read-only** so its cache-probe fingerprint matches the writer's (a mismatch only makes MCP cache reads **miss**, never serve stale).
...
- Public clustering submission also has a **per-caller admission throttle** (#535 S6) layered on the global cap: a sliding-window submit rate limit keyed on the client IP. `CLUSTERING_SUBMIT_PER_CALLER_MAX` (default `5`) submissions per `CLUSTERING_SUBMIT_WINDOW_SECONDS` (default `60`); over the limit the public submit routes (`POST /api/jobs/clustering/submit`, `POST /api/jobs/phenotype_clustering/submit`) return `429` + `Retry-After` (`RATE_LIMITED`). The throttle runs **first**, before any DB/cache work. There is deliberately **no env kill-switch**: a stray or invalid `CLUSTERING_SUBMIT_PER_CALLER_MAX` (including `0`, negative, or non-numeric) falls back to the safe default `5` rather than silently disabling the control, and all limits are clamped to sane ceilings so a typo cannot make per-caller state unbounded. The client IP is the hop the trusted proxy appended to `X-Forwarded-For`, taken from the **right** (`CLUSTERING_SUBMIT_TRUSTED_PROXY_HOPS`, default `1` = Traefik); a non-IP value in the selected hop is discarded (falls back to `REMOTE_ADDR`), and IPv6 is grouped to its `/64` so an allocation is one caller. Under a rotation flood, memory stays bounded at `CLUSTERING_SUBMIT_MAX_TRACKED` (default `20000`) fingerprints — brand-new callers collapse into one shared overflow bucket instead of evicting an active caller. On any internal throttle error the guard **fails closed** (`503 THROTTLE_UNAVAILABLE` + `Retry-After`).
  - **Trust-boundary requirement (enforced in Compose):** the throttle's non-spoofability depends on the edge proxy owning `X-Forwarded-For`. In the shipped Compose, **Traefik is the direct edge** (it publishes `:80`), so it appends the real peer address and the rightmost hop is the true client — `CLUSTERING_SUBMIT_TRUSTED_PROXY_HOPS=1` (default) is correct. Plumber folds the underscore aliases `X_Forwarded_For` / `X-Forwarded_For` / `X_Forwarded-For` into the *same* CGI field as the canonical header, so a client could otherwise inject a rotating IP that collides with the proxy value; the `api` router therefore carries an `api-strip-xff-alias` Traefik headers middleware (in `docker-compose.yml`) that **deletes those underscore aliases** before forwarding.
  - **If you deploy behind an institutional reverse proxy / load balancer in front of Traefik** (so Traefik's direct peer is that proxy, not the client), you MUST do both, or *every* client fingerprints as the shared front-proxy address and one abusive caller `429`s everyone (collateral throttling): (1) allowlist the front-proxy's source CIDRs on the Traefik web entrypoint — `--entryPoints.web.forwardedHeaders.trustedIPs=<proxy-cidr,...>` — so Traefik trusts and preserves the client IP the front proxy set instead of overwriting it with the proxy's own address; and (2) set `CLUSTERING_SUBMIT_TRUSTED_PROXY_HOPS` to the real number of trusted proxies (clamped to 8). Verify with the live two-proxy path (two distinct client IPs must throttle independently). Merely bumping `TRUSTED_PROXY_HOPS` without the Traefik `trustedIPs` allowlist does **not** work — Traefik will not have preserved a chain it did not trust. Also restrict direct ingress to Traefik's port so a client cannot bypass the front proxy.
  - **Multi-replica caveat:** the per-caller window is in-memory per API process. In the single-API Compose topology this is exact. With multiple API replicas each enforces the limit independently (effective per-caller ceiling ≈ `MAX × replicas`); the DB-backed `ASYNC_PUBLIC_JOB_CAP` remains the cross-process backstop (it is now also validated so an invalid value cannot silently disable it). For a hard cross-replica per-caller limit, add a rate-limit middleware at the single Traefik layer or move the counter to the DB.
