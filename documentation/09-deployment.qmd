...
The sidecar initializes with SysNDD-specific client instructions that describe the gene -> entity -> publication workflow, resource semantics, error envelope, and read-only constraints. Publication tools expose `recommended_citation`, `pubmed_publication_date`, `abstract_available`, and separate `sysndd_curation_date` values for linked entities. Use `get_entities_context` for 1-20 entity IDs and `get_publications_context` for 2-20 PMIDs instead of issuing many single-record calls.

The sidecar patches `mcptools` so `tools/list` advertises read-only annotations and output schemas, and so `resources/list` / `resources/read` serve the static `sysndd://schema/overview` and `sysndd://schema/tool-guide` resources. Recoverable validation failures return stable tool-result JSON envelopes with `isError = true`; malformed user inputs should not surface as JSON-RPC `-32603` internal errors.
