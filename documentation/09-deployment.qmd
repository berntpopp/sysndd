...
- `LOG_CLEANUP_AT=03:00` — daily run time, `HH:MM` in container (UTC) time (validated strictly; an invalid value refuses to start rather than tight-looping the destructive scripts). If the daily database backup runs at the same hour, set this to a non-overlapping time so a first-run backlog does not add delete/undo I/O pressure during the backup.
