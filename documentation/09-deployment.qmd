...
The sidecar initializes with SysNDD-specific client instructions that describe the gene -> entity -> publication workflow, resource semantics, error envelope, and read-only constraints. Tool descriptions include short example calls, and `get_gene_context` accepts deprecated `symbol` and `query` aliases for client self-correction. Publication tools expose `recommended_citation`, `pubmed_publication_date`, `pubmed_publication_date_confidence`, `abstract_available`, and separate `sysndd_curation_date` values for linked entities. Use `get_entities_context` for 1-20 entity IDs and `get_publications_context` for 1-20 PMIDs instead of issuing many single-record calls.

The sidecar patches `mcptools` so `tools/list` advertises read-only annotations and output schemas, and so `resources/list` / `resources/read` serve distinct static `sysndd://schema/overview` and `sysndd://schema/tool-guide` resources. Recoverable validation failures return stable tool-result JSON envelopes with `isError = true`; malformed or unknown user inputs should not surface as JSON-RPC `-32603` internal errors.
...
- Keep the MCP service on internal/private access unless a protected route is deliberately configured. MCP tools are read-only and must not call Gemini, external providers, raw SQL/R execution, write routes, admin/user/log/job routes, draft reviews, or re-review data.
