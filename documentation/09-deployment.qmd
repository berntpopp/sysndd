...
is surfaced in the restore job's `result_json` (`post_restore_scrub`). Set
`ASYNC_JOB_PAYLOAD_SCRUB_ON_STARTUP=false` only to disable the startup scrub
(default on). The remaining durable job families (publication/hgnc/ontology/
