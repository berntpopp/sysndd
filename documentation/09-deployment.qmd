...
Cache-backed MCP analysis sections depend on prewarmed API/worker-derived artifacts. The sidecar binds the same memoised wrapper names used by the API but must not call `bootstrap_init_cache_version()`, clear cache files, compute STRING networks, run phenotype clustering, or generate LLM summaries. If `dry_run` reports `temporarily_unavailable`, warm the corresponding API/admin analysis path before expecting MCP to return records.
