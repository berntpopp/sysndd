...
The public edge is deliberately bounded. A shared Host bucket permits 120 requests per minute with a 20-request burst. Because production TLS terminates at an institutional proxy, this is shared service capacity rather than a per-person quota; clients receiving HTTP 429 should retry with backoff and cache stable results. POST requests are limited to 256 KiB and 4 concurrent POST requests for the public Host bucket; excess body size returns HTTP 413. Traefik applies a 60-second whole-request read timeout. These controls provide admission and resource bounds, not authorization.
...
Access logs are enabled only for the two MCP protocol routers; the public app and Traefik's internal API/ping entrypoint are opted out. Entries retain method, path, duration, router, and response status needed to diagnose 413/429/5xx responses. Bodies, headers, query parameters, `ClientAddr`, and `ClientHost` are dropped, so request content and caller IP addresses are not retained. Log-file retention is bounded by the Traefik container's `json-file` rotation (`10m` × 3 files). Keep these privacy defaults when changing observability. Run `make test-mcp-edge` after routing or middleware changes; the disposable smoke stack exercises browser routing, protocol POST, Accept matching, GET `405`, body `413`, rate-limit `429`, router log scope, and sensitive-field omission using the resolved production image, command, and labels.
