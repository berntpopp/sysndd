...
- **geisinger_DBD** was repointed from the retired `dbd.geisingeradmi.org` CSV (404) to NDD GeneHub (`https://nddgenehub.org/files/Full-Data.csv`) by migration `038`; the parser was rewritten for that schema. This was the single broken source that had been blocking every comparison refresh.
...
- The refresh runs in the API's mirai daemon pool, which is sourced at API startup — **restart the API container** after deploying comparisons code changes before triggering a refresh.
