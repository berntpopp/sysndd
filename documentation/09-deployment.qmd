...
```bash
# 1. Redact historical backup job payloads (idempotent; also runs at startup):
docker exec sysndd-api-1 Rscript /app/scripts/scrub-job-payload-credentials.R

# 2. ROTATE the DB password, update the deployed .env / secret, then restart:
...
change — the scrub cannot rewrite old backup files. Set
