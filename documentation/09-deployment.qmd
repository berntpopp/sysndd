...
- `snapshot_stale`: an active snapshot exists but is past `stale_after`; continue serving only if acceptable for the deployment window, then refresh.
- `source_version_mismatch`: the stored source data version no longer matches the cheap current source version; refresh before treating the payload as current.
