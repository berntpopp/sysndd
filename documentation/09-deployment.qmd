...
- Public clustering submission also has a **per-caller admission throttle** (#535 S6) layered on the global cap: a sliding-window submit rate limit keyed on the client IP. `CLUSTERING_SUBMIT_PER_CALLER_MAX` (default `5`) submissions per `CLUSTERING_SUBMIT_WINDOW_SECONDS` (default `60`); over the limit the public submit routes (`POST /api/jobs/clustering/submit`, `POST /api/jobs/phenotype_clustering/submit`) return `429` + `Retry-After` (`RATE_LIMITED`). The throttle runs **first**, before any DB/cache work. Setting `CLUSTERING_SUBMIT_PER_CALLER_MAX=0` disables it (leaving only the global cap); invalid/negative values fall back to the safe default. The client IP is the hop the trusted proxy appended to `X-Forwarded-For`, taken from the **right** (`CLUSTERING_SUBMIT_TRUSTED_PROXY_HOPS`, default `1` = Traefik); IPv6 is grouped to its `/64`.
  - **Trust-boundary requirement:** the throttle's non-spoofability depends on the edge proxy owning `X-Forwarded-For`. Traefik must be the *only* thing that writes it (it appends the real peer address), and client-supplied `X-Forwarded-*` (including the `X_Forwarded_For` underscore alias that Plumber folds into the same header) must not reach the API unfiltered. In this Compose topology Traefik terminates every public request, so the rightmost hop is trustworthy; if you front the API with an additional proxy, set `CLUSTERING_SUBMIT_TRUSTED_PROXY_HOPS` to the real number of trusted proxies and ensure each strips untrusted forwarding headers. A non-IP value in the selected hop is discarded and the throttle falls back to the (unspoofable but shared) `REMOTE_ADDR`.
  - **Multi-replica caveat:** the per-caller window is in-memory per API process. In the single-API Compose topology this is exact. With multiple API replicas each enforces the limit independently (effective per-caller ceiling ≈ `MAX × replicas`); the DB-backed `ASYNC_PUBLIC_JOB_CAP` remains the cross-process backstop. For a hard cross-replica per-caller limit, add a rate-limit middleware at the single Traefik layer or move the counter to the DB.
