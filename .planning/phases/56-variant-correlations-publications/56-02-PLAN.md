...
must_haves:
  truths:
    ...
    - "Publications table prevents duplicate API calls on remount"
...
<task type="auto">
  <name>Task 1: Enhance PublicationsNDDTable with Entities table patterns</name>
  <files>app/src/components/analyses/PublicationsNDDTable.vue</files>
  <action>
Refactor PublicationsNDDTable.vue to match TablesEntities.vue patterns exactly. Key changes:
...
2. **Add isInitializing flag** in data():
```javascript
data() {
  return {
    isInitializing: true,
...
3. **Add fields_details array** in data() for expandable row details:
```javascript
fields_details: [
...
4. **Add details column** to fields array:
```javascript
{
  key: 'details',
  label: 'Details',
...
5. **Guard watchers** with isInitializing flag:
```javascript
watch: {
  filter: {
    handler() {
...
6. **Update mounted()** to use $nextTick pattern:
```javascript
mounted() {
...
7. **Add debounced loadData method** wrapping loadTableData:
```javascript
loadData() {
...
8. **Add module-level caching** to loadTableData:
```javascript
async loadTableData() {
...
9. **Extract applyApiResponse method** from loadTableData for reuse with cached data.

10. **Add PMID link** in template - update publication_id slot to link to PubMed:
```vue
...
11. **Update filtered() calls** to use loadData() instead of loadTableData() directly.
  </action>
  <verify>
...
4. Manual: Refresh page - no duplicate API calls in Network tab
5. Manual: Click PMID badge - opens PubMed in new tab
  </verify>
  <done>
Publications table has expandable row details with metadata, module-level API caching preventing duplicate calls, and PMID links to external PubMed.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add interactivity features to PublicationsNDDTimePlot</name>
...
2. **Add controls row** in template (after existing plotMode selector):
```vue
...
3. **Add aggregation helper method**:
```javascript
...
4. **Update generateLinePlot** to use aggregation:
```javascript
generateLinePlot(dataArr, dateKey) {
  ...
  const svg = d3.select('#pubs_dataviz')
    .append('svg')
...
5. **Add tooltip cleanup** at start of generateBarPlot as well:
```javascript
...
6. **Improve tooltip styling** (update existing tooltip code):
```javascript
...
7. **Update tooltip content** to show aggregation level:
```javascript
...
1. Run `cd app && npm run lint` - no linting errors
2. Run `cd app && npm run type-check` - no TypeScript errors
3. Manual: Navigate to Publications TimePlot, select "Year" aggregation - chart updates to show yearly totals
4. Manual: Toggle "Cumulative View" on - chart shows running total line
5. Manual: Select "Quarter" aggregation - chart shows quarterly data points
6. Manual: Select "Publication Type (Bar)" mode - aggregation controls are disabled
  </verify>
  <done>
TimePlot has time aggregation selector (year/month/quarter) and cumulative view toggle. Controls are disabled when viewing bar chart mode. Tooltips show appropriate labels based on view mode.
  </done>
</task>

<task type="auto">
  <name>Task 3: Add metrics cards to PublicationsNDDStats</name>
...
computed: {
  metricsCards() {
    ...
    return [
      ...
      {
        title: `Publications ${currentYear}`,
...
5. Manual: Verify "Publications [current year]" shows publications from current year
6. Manual: Verify "YoY Growth" shows percentage change
7. Manual: While loading, skeleton cards appear instead of empty space
  </verify>
  <done>
Stats view displays metrics cards showing Total Publications, Publications this year, Year-over-Year growth rate, and Newest publication date. Cards use Bootstrap icons and color-coded variants.
  </done>
</task>
...
6. Click PMID badge - PubMed opens in new tab
7. Verify pagination still works
8. Verify column sorting still works
9. Verify search/filter still works
...
4. Verify "Publications [year]" shows current year publications
...
4. PUB-04: Stats view displays metrics cards with publication counts and growth rate
