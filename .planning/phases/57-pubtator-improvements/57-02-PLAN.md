...
must_haves:
  ...
  key_links:
    ...
    - from: "app/src/views/analyses/PubtatorNDD.vue"
      to: "PubtatorNDDGenes.vue"
      via: "emitted novelGeneCount event"
      pattern: "@novel-count"
---
...
1. **Convert to Composition API** (`<script setup lang="ts">`):
   ...
   - Define emits with defineEmits for `@novel-count`
...
4. **Pass event handler to child** (if using named slot or provide/inject):
   - Since this uses router-view, use provide/inject pattern:
   ```typescript
   provide('onNovelCount', handleNovelCount);
   ```

   In PubtatorNDDGenes.vue:
   ```typescript
   const onNovelCount = inject<(count: number) => void>('onNovelCount');
   watch(novelCount, (count) => onNovelCount?.(count), { immediate: true });
   ```
...
5. Help documentation explains Pubtator concept and features (PUBT-06)
