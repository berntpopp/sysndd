...
must_haves:
  truths:
    ...
    - "Approving a re-review through the dedicated endpoint produces the same database outcome as approving through the standard review/status endpoints"
...
<task type="auto">
  ...
  <verify>
    ...
    Verify `grep "conn = conn" api/functions/review-repository.R` confirms transaction participation in sync call.
    Verify `grep "conn = conn" api/functions/status-repository.R` confirms transaction participation in sync call.
  ...
  <done>
    ...
    - review_approve() calls sync inside its transaction block with conn = conn
    - status_approve() calls sync inside its transaction block with conn = conn
