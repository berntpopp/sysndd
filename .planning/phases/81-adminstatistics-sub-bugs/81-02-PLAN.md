...
files_modified:
  ...
  - app/tests/unit/utils/dateUtils.spec.ts
  - app/tests/unit/utils/apiUtils.spec.ts
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/tests/unit/utils/dateUtils.spec.ts"
      provides: "Unit tests for date utilities"
      contains: "inclusiveDayCount"
    - path: "app/tests/unit/utils/apiUtils.spec.ts"
  ...
  key_links:
    ...
    - from: "app/src/views/admin/AdminStatistics.vue"
      to: "AbortController"
      via: "trendAbortController created per request, abort on granularity change"
...
<task type="auto">
  ...
  <action>
    ...
    3. Create `app/tests/unit/utils/dateUtils.spec.ts` with tests:
    ...
    4. Create `app/tests/unit/utils/apiUtils.spec.ts` with tests:
  ...
  <verify>
    Run `cd /home/bernt-popp/development/sysndd/app && npx vitest run tests/unit/utils/dateUtils.spec.ts tests/unit/utils/apiUtils.spec.ts` -- all tests pass.
...
<task type="auto">
  ...
  <action>
    ...
    c. After computing cumulative trendData, derive totalEntities INSIDE fetchTrendData (move from fetchKPIStats):
    ```typescript
    ...
    12. In `ReReviewBarChart.vue`, the clampPositive via `Math.max(0, ...)` was already applied in Plan 81-01's Task 2. Verify it is present. If not, ensure the Pending Review and Not Yet Submitted datasets use `Math.max(0, ...)` wrapping.
  </action>
  <verify>
    Run `cd /home/bernt-popp/development/sysndd/app && npx vue-tsc --noEmit` -- no TypeScript errors.
  ...
  <done>
    ...
    - onUnmounted() cleans up AbortController
...
<verification>
1. All utility tests pass: `cd /home/bernt-popp/development/sysndd/app && npx vitest run tests/unit/utils/`
...
5. Verify AbortController lifecycle: new controller per request, abort on re-fetch and unmount
...
- AbortController cancels stale trend requests on granularity change
