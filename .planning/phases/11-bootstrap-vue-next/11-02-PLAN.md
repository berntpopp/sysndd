...
```
  ...
  <verify>
Run `grep -r "\\$root.\\$emit" app/src --include="*.vue"` - should return 0 results related to modals.
Test modal functionality in Curate and Review views if accessible.
  </verify>
  <done>
All $root.$emit('bv::show::modal') and $root.$emit('bv::hide::modal') patterns replaced. Modals open and close correctly using composable or v-model.
  </done>
</task>
