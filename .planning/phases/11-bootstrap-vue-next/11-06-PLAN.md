...
must_haves:
  truths:
    ...
    - "Form validation works with vee-validate 4"
    - "vue-treeselect dropdowns work"
    - "No vue2-perfect-scrollbar usage (native scrollbars)"
    - "UpSet.js chart renders (using @upsetjs/bundle)"
...
<task type="auto">
  <name>Task 2: Migrate vee-validate 3 to 4 and third-party components</name>
  <files>app/src/main.js, app/src/views/Login.vue, app/src/views/Register.vue, app/src/views/PasswordReset.vue, app/src/components with vue-treeselect</files>
  <action>
Migrate vee-validate and vue-treeselect:

**VeeValidate 3 → 4 (3 files):**
...
**Vue-treeselect → @r2rka/vue3-treeselect (12 files):**
...
2. Update imports in each of 12 files:
   ```javascript
   // Before
...
3. Component name in template may need update:
   ```vue
   <!-- Before -->
...
Search: `grep -r "vue-treeselect\|Treeselect" app/src --include="*.vue" -l`
  </action>
  <verify>
- Login form: Fill and submit, verify validation messages appear/disappear
- Components with treeselect: Verify dropdown opens and selection works
  </verify>
  <done>
vee-validate 4 configured with Composition API. Vue-treeselect replaced with vue3-treeselect. Forms validate correctly, treeselects work.
  </done>
</task>

<task type="auto">
  <name>Task 3: Remove perfect-scrollbar and update UpSet.js</name>
