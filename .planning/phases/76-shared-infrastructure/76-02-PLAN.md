...
must_haves:
  truths:
    - "parse_genemap2() extracts disease_ontology_name, MIM_Number, Mapping_key, and hpo_mode_of_inheritance_term_name from genemap2.txt Phenotypes column"
...
4. **Select relevant columns**: `dplyr::select(Approved_Symbol, MIM_Number, Phenotypes)`
   (Keep MIM_Number at gene level too -- needed for ontology system)
...
5. **Multi-stage Phenotypes parsing** (copy from comparisons-functions.R lines 429-446):
   ...
   - `separate(disease_ontology_name, c("disease_ontology_name", "disease_MIM_Number"), ", (?=[0-9]{6})", fill = "right")` -- extract disease MIM (rename to disease_MIM_Number to distinguish from gene MIM_Number)
   - Clean whitespace in Mapping_key and disease_MIM_Number
   - `filter(!is.na(disease_MIM_Number))` -- remove entries without disease MIM
   - `filter(!is.na(Approved_Symbol))` -- remove entries without gene symbol
   - `mutate(disease_ontology_id = paste0("OMIM:", disease_MIM_Number))`
...
6. **Inheritance term processing** (copy from comparisons-functions.R lines 444-466):
   ...
   - Remove disease_MIM_Number column (now captured in disease_ontology_id)
   - `unique()` -- deduplicate
