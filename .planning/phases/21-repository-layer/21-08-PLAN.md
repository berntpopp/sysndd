...
must_haves:
  truths:
    ...
    - "API starts successfully and responds to requests"
  artifacts:
...
<task type="auto">
  <name>Task 3: Verify zero dbConnect calls remain</name>
  <files>N/A - verification only</files>
  <action>
Run comprehensive verification to ensure no dbConnect() calls remain:

```bash
# Count all dbConnect in api/ directory
grep -r "dbConnect" api/ --include="*.R" | grep -v "start_sysndd_api.R" | grep -v "tests/" | wc -l

# If any found, list them:
grep -rn "dbConnect" api/ --include="*.R" | grep -v "start_sysndd_api.R" | grep -v "tests/"
```

Expected result: 0 occurrences (excluding start_sysndd_api.R pool creation and test files)

Also verify API starts:
```bash
cd api && timeout 10 Rscript -e "source('start_sysndd_api.R')" 2>&1 | head -20
```
  </action>
  <verify>
- grep -r "dbConnect" api/ --include="*.R" | grep -v "start_sysndd_api.R" | grep -v "tests/" | wc -l returns 0
- API initialization doesn't error
  </verify>
  <done>
- Zero dbConnect calls in production code (only pool creation and tests)
