...
<task type="auto">
  ...
  **Change 3: Skip update_source_last_updated() for omim_genemap2 (around lines 963-966)**
  ...
  6. The old inline genemap2 parsing code is gone: `grep -c "Chromosome = X1" api/functions/comparisons-functions.R` returns 0
  7. The old inline inheritance case_when is gone: verify no "Autosomal dominant.*Autosomal dominant inheritance" pattern exists in the adapter function
...
3. No duplicate genemap2 parsing code: `grep -c "Chromosome = X1" api/functions/comparisons-functions.R` is 0
...
10. Lint check: `cd api && Rscript -e "lintr::lint('functions/comparisons-functions.R')"` produces 0 errors
11. Lint check: `cd api && Rscript -e "lintr::lint('functions/omim-functions.R')"` produces 0 errors
</verification>
