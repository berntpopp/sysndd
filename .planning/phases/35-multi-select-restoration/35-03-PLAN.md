...
- Selected items display as chips below selector
...
3. Verify phenotype selector:
   - Click to open dropdown - should show hierarchical tree
   - Expand a category (e.g., "Abnormality of the nervous system")
   - Check multiple items - checkboxes should work
   - Verify chips appear below selector showing selected items
...
- Multiple items can be selected via checkboxes
...
2. `grep -r "TreeMultiSelect" app/src/views/` shows usage in all 3 views
3. `cd /home/bernt-popp/development/sysndd/app && npm run build` succeeds
4. Human verified multi-select works in Review, ModifyEntity, ApproveReview
</verification>
