...
must_haves:
  truths:
    ...
    - "Admin can GET /api/llm/prompts to get all prompt templates"
    - "Admin can PUT /api/llm/prompts/:type to update a prompt template"
    - "All endpoints require Administrator role"
  artifacts:
    - path: "api/endpoints/llm_admin_endpoints.R"
      provides: "All LLM admin API endpoints"
      exports: ["GET /config", "PUT /config", "GET /cache/stats", "GET /cache/summaries", "DELETE /cache", "POST /regenerate", "GET /logs", "POST /cache/:id/validate", "GET /prompts", "PUT /prompts/:type"]
...
Output: Complete REST API at /api/llm/* with config, cache, logs, prompts, and regeneration endpoints
</objective>
...
@api/functions/llm-batch-generator.R (trigger_llm_batch_generation function)
...
6. **POST /regenerate** - Trigger regeneration job
   ...
   - **Important:** Call trigger_llm_batch_generation() from api/functions/llm-batch-generator.R
     - This function already exists (created in Phase 59) with signature:
       `trigger_llm_batch_generation(clusters, cluster_type, parent_job_id)`
     - Pre-fetch cluster data from database (pool not available in daemon)
     - The function internally uses create_job() with operation = "llm_generation"
     - Pass clusters tibble, cluster_type string, and a generated job_id as parent_job_id
...
9. **GET /prompts** - Get all prompt templates
   - require_role(req, res, "Administrator")
   - Call get_all_prompt_templates() from llm-service.R
   - Returns all 4 prompt types with their active templates

10. **PUT /prompts/<type>** - Update a prompt template
    - require_role(req, res, "Administrator")
    - Validate type against valid_types (functional_generation, functional_judge, phenotype_generation, phenotype_judge)
    - Parse body for template, version, description
    - Get user_id from get_user_id_from_token(req)
    - Call save_prompt_template(type, template, version, description, created_by)
    - Return success + type + version + template_id
...
Should output number >= 10 (endpoints parsed successfully).
  </verify>
  <done>
llm_admin_endpoints.R exists with 10 endpoints including prompt endpoints, can be parsed by Plumber without errors.
  </done>
</task>
...
2. llm_admin_endpoints.R parses successfully with Plumber (10 endpoints)
