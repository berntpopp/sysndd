...
must_haves:
  ...
  key_links:
    - from: "app/src/views/admin/ManageLLM.vue"
      ...
      pattern: "useLlmAdmin"
    - from: "app/src/assets/js/constants/main_nav_constants.ts"
...
<task type="auto">
  <name>Task 4: Add navigation link to main_nav_constants.ts and run linting</name>
  <files>app/src/assets/js/constants/main_nav_constants.ts</files>
  <action>
1. **Add ManageLLM to admin navigation** in app/src/assets/js/constants/main_nav_constants.ts:
   - Find the `administration_dropdown` section in `DROPDOWN_ITEMS_RIGHT` array (around line 86-106)
   - This is the admin dropdown with items like ManageUser, ManageAnnotations, ManageBackups, etc.
   - Add new menu item after 'Manage PubTator' (line 104) and before the closing bracket of the items array:
   ```typescript
   { text: 'LLM Management', path: '/ManageLLM', icons: ['gear', 'robot'] },
   ```
   - The exact location is after line 104 which reads:
...
Verify that main_nav_constants.ts contains path: '/ManageLLM'
  </verify>
  <done>
ManageLLM navigation link added to administration dropdown in main_nav_constants.ts, all frontend linting and type-check pass.
  </done>
</task>
...
- Navigation link in admin menu (Administration dropdown)
  </what-built>
  <how-to-verify>
1. Start development environment: make dev
2. Log in as Administrator
3. Navigate to Admin menu (top right) -> LLM Management
...
3. Navigation link added to main_nav_constants.ts administration_dropdown
...
<success_criteria>
- Admin can navigate to /ManageLLM from header Administration dropdown
