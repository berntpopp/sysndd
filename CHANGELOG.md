...
- **Phenotype clustering no longer releases the worse of two k-means optima.** The MCA/HCPC consolidation ran k-means once, from the Ward-cut centroids. On the public phenotype snapshot 196 that start converged to the higher-inertia of two local optima (within-cluster inertia 0.3603 vs 0.3530, mean silhouette 0.145 vs 0.183), and removing a single entity could flip the released partition between them — which is why the partition re-split between two snapshots on near-identical input. The consolidation now runs from the Ward-cut centroids **and** from 100 seeded random starts and releases the lowest within-cluster inertia, deterministically (ties: Ward-cut start, then seed order). Deleting 20 random entities (1%) 40 times over, the legacy procedure flipped basin (ARI ≈ 0.35) in most trials; the new one holds k = 3 in 39 of 40 and stays at ARI ≥ 0.93 whenever k holds. What multi-start cannot fix is that the k rule itself is nearly tied on this input (W(3)/W(2) = 0.901 vs W(4)/W(3) = 0.914): in the remaining trial it tipped to k = 4. That margin is now served (`k_selection`). `iter.max` was raised from 10 to 100 and convergence is recorded. **Membership changes**: `CLUSTER_LOGIC_VERSION` is bumped, so deploy with the membership-change runbook (restart workers, force-refresh `phenotype_clusters` then `phenotype_functional_correlations`, force-regenerate phenotype LLM summaries). Existing snapshots are not rewritten. (#679)
...
- **FactoMineR 2.13 → 2.17** (adds `irlba`, `showtext`, `sysfonts`, `showtextdb`). 2.17 switches to a truncated, randomly-started SVD (irlba) when `ncp` is small, which on production-shaped input is only approximate (~3e-4 in the coordinates), seed-dependent, and truncates `eig`; `phenotype_mca_fit()` therefore always requests the full spectrum and keeps the leading 8 columns — exact, seed-free, and bit-identical to what 2.13 returned — for the served partition, the validator and the reproducibility bundle alike. The cluster descriptions go through a version-proof `catdes` wrapper (2.17 adds an `n` column and an `html.table` argument), a cluster with no significant description no longer crashes the clustering call, and the FactoMineR version is part of the phenotype cache fingerprint. `test-unit-phenotype-package-drift-guard.R` pins coordinates, k, the partition and the served shapes across package updates. (#679)

### Added

- **Optimisation landscape, selector curve and continuity in the phenotype validation block** (`validation_schema_version` 2.1): `consolidation_landscape` (number of starts, basins vs micro-variants, chosen and runner-up inertia, share of starts reaching each, whether the Ward-cut start reached the chosen basin), `k_ward_ratio_curve` (the quantity k is actually chosen by), `k_selection` (runner-up k and the ratio margin — how contested k itself was), `continuity` (adjusted Rand index and per-cluster Jaccard against the public snapshot being superseded), `procedure_version`, `k_rule` and `factominer_version`. All additive and excluded from `payload_hash`. The snapshot's applied-params block and the reproducibility bundle record the procedure version, k rule, multi-start configuration, k-means algorithm, and the FactoMineR and R versions. New env `ANALYSIS_PHENOTYPE_CONSOLIDATION_STARTS` (default 100; part of the phenotype cache fingerprint). (#679)
...
- **MCP protocol compatibility is explicit** (#629). Operational probes enforce negotiated `2025-11-25` while production also accepts `2025-06-18`; documentation records the newer breaking `2026-07-28` revision as a separate future runtime/client/routing migration rather than implying that public exposure upgrades the wire protocol.
...
- **`GET /api/comparisons/crosswalk`** (`api/endpoints/comparisons_endpoints.R`): a pure, DB-free, unauthenticated endpoint returning the declarative source→normalized-tier crosswalk (`comparison_category_crosswalk()`, `api/functions/category-normalization.R`) plus the `mapping_version`. It is the authoritative, versioned mapping table; a CI guard (`api/tests/testthat/test-unit-comparisons-crosswalk.R`) drives the executable normalizer from every crosswalk row so the published mapping can never drift from the code.
- **Curation Comparisons tier-mapping help** (`app/src/components/analyses/EvidenceTierMappingHelp.vue`): a keyboard-accessible question-mark affordance immediately after the `Overlap` label on `/CurationComparisons`. Its concise popover is rendered entirely from `/api/comparisons/crosswalk` (no hard-coded mapping text), shows the `mapping_version`, and links to the complete crosswalk; Escape closes it and restores focus.
- **Additive analysis-snapshot generator provenance** (migration `046_add_analysis_snapshot_generator_provenance.sql` → `analysis_snapshot_manifest.generator_json`): each newly built snapshot records its application version + git commit, `CLUSTER_LOGIC_VERSION`, snapshot-builder version, generation timestamp (single UTC), applied algorithm parameters, and clustering library versions. Surfaced at `meta.snapshot.generator` with a companion `generator_hash`, and recorded per layer in analysis-snapshot releases. A build-time gate rejects incomplete provenance.

### Changed

- **PanelApp normalized crosswalk is now a full ordinal** (`api/functions/category-normalization.R`): Green/`3`→Definitive, **Amber/`2`→Moderate** (was Limited), **Red/`1`→Limited** (was Refuted). PanelApp Red denotes low evidence, not affirmative refutation, so no PanelApp tier maps to Refuted; explicit source-native disputed/refuted assertions (e.g. Gene2Phenotype) still map to Refuted. The `mapping_version` (`2026-07-19.583-panelapp-ordinal`) is now carried on `GET /api/comparisons/metadata` and on the browse response `meta` and XLSX export, so a frozen downstream comparison can identify the policy that produced it. This is a **read-time** mapping — an **API restart** applies it (no comparisons refresh, no worker restart). Analysis snapshots, clustering, richness/coverage, and the MCP raw-category projection are unaffected.
- Generator provenance is **additive only**: it lives outside `payload_hash`/`input_hash`/`cluster_hash`, so it changes no cluster membership, `cluster_hash`, LLM summary, or release `content_digest`, and **`CLUSTER_LOGIC_VERSION` is unchanged** (no LLM regeneration). Optional provenance backfill: restart `worker` and `worker-maintenance` (the builder is worker-executed), force-refresh both cluster presets, then force-refresh `phenotype_functional_correlations` (dependency gate).
