...
- **Curation comparisons Table filters reverted to stale results** (#467): the table applied every browse response with no request coordination, so a slow earlier (unfiltered) request could resolve after a newer filtered one and clobber it ("filter to a gene, then it reverts"). Loads now go through the shared `createTableRequestCoordinator` (dedupe + `isCurrent` stale-drop), matching the Entities/Genes/Phenotypes tables. The same stale-response race was swept and hardened across `PubtatorNDDTable`, `PublicationsNDDTable`, and the admin `ManageOntology` table (the last two had hand-rolled dedup but no stale guard). Regression test added.
