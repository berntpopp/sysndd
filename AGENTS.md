...
- External proxy fetchers must use `memoise_external_success_only()` rather than raw `memoise::memoise()`. Successful and true not-found responses may be cached, but `list(error = TRUE, ...)` transient upstream failures must not poison the 7/14/30-day external caches. Pass `source = "<provider>"` to the wrapper so it emits one structured `[external-proxy] ... event=complete status=... elapsed_ms=... cache=hit|miss` timing log per call (used by `gnomad`/`ensembl`/`uniprot`/`alphafold`); `mgi`/`rgd` instead log via the inline `external_proxy_with_timing()` wrapper inside the fetcher, so they omit the memoise `source` to avoid double-logging. Per-request budgets fast-fail a slow upstream so a single request cannot occupy a worker for tens of seconds; true heavy/light worker-pool isolation is tracked in #154. Static guards: `api/tests/testthat/test-unit-external-proxy-budgets.R` and `api/tests/testthat/test-unit-external-slow-provider.R`.
